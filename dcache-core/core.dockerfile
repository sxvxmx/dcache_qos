...
# Configuration files
RUN mkdir -p /var/lib/dcache/config
COPY config/poolmanager.conf /var/lib/dcache/config/poolmanager.conf
...
RUN service postgresql start && \
    ...
    su - postgres -c "createdb -O dcache qos" && \
    su - postgres -c "createdb -O dcache pinmanager" &&\
    # Update pg_hba.conf
